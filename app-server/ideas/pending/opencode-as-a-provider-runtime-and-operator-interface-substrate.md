# Idea: OpenCode as a Provider, Runtime, and Operator-Interface Substrate

**Status:** Idea / reconnaissance candidate  
**Decision type:** Architecture exploration  
**Primary question:** Can Work Engine preserve authority over orchestration and context lifecycle while reusing OpenCode for provider integration, execution/runtime machinery, and operator-interface infrastructure?

## Summary

Investigate OpenCode as an open-source substrate beneath Work Engine rather than treating it as a wholesale replacement for either Work Engine or Codex App Server.
...
# Repository Evaluation Addendum — 2026-09-05

## Status and attribution

This addendum preserves a read-only Codex repository evaluation performed on
2026-09-05. It narrows the preceding hypothesis; it does not replace the
original idea, accept an architecture, authorize implementation, change the
...
Do not place provider transport, harness execution, and operator presentation
behind one undifferentiated OpenCode runtime adapter. Preserve three ports:
...
`ProviderTurnPort` should own one request/stream boundary, cancellation,
portable events, provider capability discovery, and attributed provider-native
escape hatches. It must not own Work Engine continuation, context-policy, or
workflow state.

`HarnessRuntimePort` should expose thread/session realization, harness-native
tools, sandbox and approval mechanics, native context-transition mechanisms,
and lifecycle observations without asserting false equivalence across
harnesses.

`OperatorProjection` should expose Work Engine-owned status and commands to a
replaceable UI. It must not make a UI session the canonical role, workflow, or
context identity.

The current `ManifestRoleRuntime` injection point is a useful precursor, but a
future neutral contract should not rely only on duck typing of
`deliverTurn(...)`. It should close capability discovery, event identity,
cancellation, raw-evidence custody, reconciliation, and unsupported-operation
semantics.
...
This addendum does not amend S13 or insert OpenCode into its active execution.
