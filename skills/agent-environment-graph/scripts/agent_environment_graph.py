...
def render_site(
    ...
    for role_id, role in environment["roles"].items():
        filename = _role_slug(role) + ".md"
        require(filename not in pages, f"duplicate role page filename: {filename}")
        pages[filename] = render_role_page(catalog, environment, role_id, invariant_path, environment_path)
...
def write_site(output: Path, role_output_dir: Path, overview: str, pages: dict[str, str]) -> list[str]:
    ...
    removed: list[str] = []
    for candidate in sorted(role_output_dir.glob("*.md")):
        if candidate.name in pages:
            continue
        if GENERATED_COMMENT in candidate.read_text(encoding="utf-8"):
            candidate.unlink()
...
def check_site(output: Path, role_output_dir: Path, overview: str, pages: dict[str, str]) -> None:
    ...
    unexpected = sorted(
        path.name
        for path in role_output_dir.glob("*.md")
        if path.name not in expected and GENERATED_COMMENT in path.read_text(encoding="utf-8")
    )
    require(not unexpected, f"unexpected generated role pages: {', '.join(unexpected)}")
