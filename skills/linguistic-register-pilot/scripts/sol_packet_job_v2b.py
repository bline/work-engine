#!/usr/bin/env python3
"""Run one v2b packet with a durable marker written before model launch."""
...
def main() -> int:
    ...
    require(sha256_bytes(canonical(request)) == args.request_digest, "request digest differs from launch manifest")
    ...
    emit_new(args.attempt_marker, {
        "artifact_type": "linguistic_register_v2b_attempt_marker",
        "schema_version": 1,
        **request,
    ...
    stderr_bytes = b""
    return_code = 125
    with tempfile.TemporaryDirectory(prefix=f"linguistic-register-v2b-{args.stage}-") as temporary:
    ...
    receipt = {
        "artifact_type": "linguistic_register_model_execution_receipt_v2b",
        "schema_version": 1,
        **request,
        "request_digest": args.request_digest,
        "provider": "OpenAI",
    ...
    emit_new(args.receipt_output, receipt)
...
if __name__ == "__main__":
    try:
        ...
        print(f"sol_packet_job_v2b: {error}", file=sys.stderr)
        raise SystemExit(2)
