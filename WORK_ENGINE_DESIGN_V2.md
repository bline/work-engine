...
Variant structure describes the machinery that currently exists and why it is shaped that way: available capabilities, agent topology, lifetimes, provider arrangements, component boundaries, infrastructure, and other operational mechanisms that may evolve over time.
...
Variant structure should help the model understand the machine it is operating inside without dictating how it must use that machine.
