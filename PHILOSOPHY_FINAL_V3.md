...
The useful durable consequence is not the full history of how X was chosen. It is that **Y no longer supports X under the conditions established by Z**.
...
Route revision therefore prevents inferential debt from persisting. When evidence invalidates a premise or route, the resulting stale conclusion should be retired explicitly rather than silently reconciled with later evidence. Observations and decisions that remain valid should survive; only the consequences made stale by the new evidence should be revised.
