...
Consequences describe what successful work must accomplish or what a decision must preserve, without prescribing the method used to accomplish it.

Examples:

- establish enough evidence to support a placement decision;
- detect a competing owner before committing to an architectural boundary;
- preserve review independence from the builder;
- avoid repeated reconstruction of context that is still useful;
- prove the intended runtime or user-visible behavior;
- keep direct-source observation bounded to relevant evidence;
- stop investigation when additional evidence has no credible decision value.

When a requirement is not truly invariant, prefer expressing its **consequence** rather than converting the current best method into a rule.
...
## 2. Commands are product structure
...
### 2.1 Every command needs a causal parent
...
### 2.2 Commands must encode structure, not data
...
### 2.4 Commands must not collapse semantic space unnecessarily
...
### 2.5 Sequencing is structural only when causality requires it
...
### 2.6 Exhaustive commands require genuinely closed worlds
...
### 2.7 Failure-mode explanations are knowledge, not escape hatches

Explaining why a command exists does not weaken the command.

The model remains bound by the structural invariant, but now possesses causal knowledge that can propagate through later reasoning. When a novel situation appears, the model can preserve the underlying property rather than blindly reproduce an old ritual.
...
### 2.8 Hard structure and the fuzzy map

The model operates with two different kinds of structure.

**Hard structure** is externally established by the product. It consists of the small set of commands, contracts, authority boundaries, ownership relationships, interface requirements, and other invariants that must not drift with interpretation.

**The fuzzy map** is the model's inferred understanding of the current situation: likely architecture, relevant evidence, implementation strategy, placement, risk, reviewer concerns, and other conclusions that may change as evidence changes.

The distinction matters because inferred structure is revisable. Different parts of an inferred map may be supported with different strength, and new evidence can reorganize that map. The model should not have to infer or repeatedly renegotiate the product's load-bearing structure at the same time.

Hard structural commands provide stable reference points around which the fuzzy map can change.
...
This is analogous to useful structure in ordinary human life. People act inside relatively stable physical, social, and relational structures. Those structures reduce the number of interpretations and responses that must be reconsidered at every moment. Work Engine deliberately gives its models the same kind of stable frame.

The purpose is not to eliminate uncertainty. It is to **localize uncertainty**.

When new evidence contradicts part of the fuzzy map, the model can ask:

> Given that the product's structural invariants remain fixed, what part of my current interpretation should change?

It does not need to reconstruct ownership, authority, interface semantics, and every other foundational relationship along with the changed hypothesis.

This produces an important distinction:

> **Good structural invariants reduce semantic space by eliminating invalid worlds.**
>
> **Bad procedural rules reduce semantic space by eliminating legitimate solutions.**

The first reduces complexity. The second creates brittleness.

Hard structure therefore reduces the number of invalid degrees of freedom exposed by an agent interface. The model need not spend reasoning effort deciding facts it is not authorized to change.

A well-formed interface can increasingly approach:
...
rather than exposing architectural decisions that are already fixed by the product.

A useful shorthand is:

> **Pin the coordinate system, not the path.**

Commands pin the minimum structural reference points required for coherent operation. The model remains free to construct and revise the fuzzy map between those points.

### 2.9 Structural certainty and reasoning economy

Hard structure does more than constrain behavior. It reduces the complexity of the reasoning problem.

Without structural anchors, the model may need to infer questions such as:

- who owns an artifact;
- whether a component may mutate another component's state;
- whether an output is authoritative;
- which interface shape downstream consumers accept;
- whether provenance is required;
- whether review evidence must remain independent.

Those are additional dimensions in the model's open problem space.

When the product establishes their genuinely invariant answers, those dimensions no longer need to be repeatedly inferred. Reasoning capacity can instead be spent on the dimensions where contextual judgment creates value.

Operationally, structural commands condition the model's available interpretations and actions: continuations that violate a known invariant become poor candidates, while compatible reasoning can proceed from a stable reference point. This should not be understood as a literal explicit probability table inside the model; it is a useful description of the behavioral effect of providing stable, high-certainty context.

The goal is therefore not maximum freedom and not maximum constraint.

The goal is:

> **Remove invalid degrees of freedom while preserving every legitimate degree of freedom.**

This is why structural commands can make a model-centered system more capable even though commands constrain it. Correctly chosen constraints reduce irrelevant uncertainty. Incorrectly chosen procedural constraints remove useful solution space.

### 2.10 Logical continuity connects hard structure to judgment

A structural anchor is most useful when the model also understands why it exists.

The causal explanation connects the hard structural map to the fuzzy inferred map:
...
→ structural invariant
→ minimal command
→ model reasoning in the remaining open space
```

This continuity lets the reason for an invariant propagate through later reasoning. A novel circumstance can change the fuzzy map without severing the logic that protects the hard structure.

A naked command supplies a fixed point but little information about how that point relates to unfamiliar situations. A command with its causal parent supplies both the fixed point and the relationship that makes it load-bearing.

This is the **Logical Continuity Principle**:

> Every structural command should be derivable from an explicit causal explanation, so the model can carry the reason for the invariant into subsequent reasoning rather than treating the command as an isolated policy fact.


### 2.11 Command review test
...
2. Is that property genuinely invariant, or merely desirable in the current situation?
3. Does the command encode only structure, authority, or an irreducible interface contract?
4. Has mutable data, current knowledge, a heuristic, or a preferred route leaked into the command?
5. Could a different method preserve every required consequence? If so, is the command unnecessarily prescribing that method?
6. Is any required sequencing actually causal?
7. Is the command paired with enough explanation for the model to understand why the boundary exists?
8. Would the command remain valid if tools, repository structure, models, or evidence changed while the product contract stayed the same?
...
## 4. Contracts are not procedures
...
## 5. Evidence and capability selection
...
## 6. Information lifetime is a decision dimension

Context is a resource with value and cost. Its lifetime should reflect how long the information remains useful to correct judgment.
...
## 7. Independence is a property, not ritualized amnesia
...
## 8. Determinism and model judgment
...
## 10. User authority and approval
...
## 12. Feature-design rules

Every new Work Engine feature should be designed in this order:
...
## 14. High-level procedures remain useful
