...
| `MECH-NATIVE-REVIEW-CLOSURE` | App Server stable-host native-review composition plus native Claude Code adapter | Executes only an exact configured and supervisor-selected immutable review obligation, durably preserving provider-entry admission, episode continuity, admitted result, findings, and reliance without granting the supervisor provider, mutation, finding-evaluation, or acceptance authority. | realizes `INV-012`; supports `INV-014`, `INV-017` |
